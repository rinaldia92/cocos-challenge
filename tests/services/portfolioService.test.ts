...
describe('Portfolio Service', () => {
    describe('getPortfolioService', () => {
        test('should get portfolio with orders', async () => {
            ...
            const marketData = getMarketDataByInstrumentIdsMock(orders.map((order) => order.instrumentId as number));
            const instruments = getInstrumentsByIdsMock(orders.map((order) => order.instrumentId as number));
